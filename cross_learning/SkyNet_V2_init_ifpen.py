...
for i in range(AI_va.shape[0]) : 
    coeff = PP(WSA = AI_va[i], AA = Azimuths[i])
...
T800.init_weight(path = "/home/arthur/Bureau/BEM IFPEN/BEM IFPEN/bemol-v0.0.1/cross_learning/ifpen.pt2", samp_size=samp_size, transfert = False)
...
path_param = "/home/arthur/Bureau/BEM IFPEN/BEM IFPEN/bemol-v0.0.1/learning_transfer/ifpen.pt2"
#T800.save_param("cross_learning/ifpen.pt2")
...
solver_validation = bemol.ning.NingUncoupled(mexico, rho, corr_yaw_PP)
...
for i, az in enumerate(azs):
    ...
    corr_T800 = T800(X).to("cpu").item() ; corr_ifpen = PP(AI, az) ; corr_PP = PP(AI, az)
