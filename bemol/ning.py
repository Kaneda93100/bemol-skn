...
class NingUncoupled(bem.BaseBEM):
    ...
    def solve(self,section:section.Section,azimuth:float,pitch:float, velocity:list=[0.0,0.0,0.0],angles=[0.0,0.0],tStep=0.0):
...
######## Application des correctifs sur le yaw ici ########################################
...
############################################################################################
        ...
        if isinstance(attackAngle, torch.Tensor) :
            """
            Les prochaines lignes n'ont été ajouté que pour éviter une erreur
            de définition de l'opérateur *('numpy.array', 'torch.tensor') 
            """
            liftCoeff = torch.Tensor(liftCoeff)
            dragCoeff = torch.tensor(dragCoeff)
